...
test('Запись HAR файла с реальной сессией', async ({ page }) => {
  ...
  // 1. Открываем модальное окно (клик на картинку)
  ...
  await page.waitForSelector('#modals', { timeout: 10000 });
  await page.locator('#modals button').first().click();
  
  console.log('✅ HAR файл записан!');
});
