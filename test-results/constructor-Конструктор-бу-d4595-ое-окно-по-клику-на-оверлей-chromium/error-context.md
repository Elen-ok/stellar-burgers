...
- Name: constructor.spec.ts >> Конструктор бургера >> должен закрывать модальное окно по клику на оверлей
- Location: tests\constructor.spec.ts:95:7
...
Call log:
  - waiting for locator('div').filter({ hasText: 'Биокотлета из марсианской Магнолии' }).first()
...
```ts
  ...
  19  | };
  20  | 
  21  | test.describe('Конструктор бургера', () => {
